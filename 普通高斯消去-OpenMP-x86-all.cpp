...
#pragma omp single
        {
            ...
            int j = 0;
            for (j = k + 1; j + 8 <= N; j += 8) {
                __m256 t2 = _mm256_loadu_ps(&A[k][j]);   //¦Ä??????loadu??storeu???
...
int main() {
    ...
    cal(LU);
    cout << "????????§Ü????" << (tail - head) * 1000 / freq << "ms" << endl;
    deleteA();
    ...
    cal(avx_omp_static);
    cout << "avx_OpenMP_static?????" << (tail - head) * 1000 / freq << "ms" << endl;
    deleteA();

    cal(avx_omp_dynamic);
    cout << "avx_OpenMP_dynamic?????" << (tail - head) * 1000 / freq << "ms" << endl;
    deleteA();

    cal(avx_omp_guided);
    cout << "avx_OpenMP_guided?????" << (tail - head) * 1000 / freq << "ms" << endl;
    deleteA();
}
